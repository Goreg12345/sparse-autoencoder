...
parser.add_argument("--store_size", type=float, help="Number of activations in store, should be big, e.g. 1e9 to have enough activations for training")
...
if __name__=='__main__':
    ...
    with h5py.File(os.path.join(cfg['store_path'], filename), 'w') as f:
        ...
        dset = f.create_dataset('tensor', (cfg['store_size'], cfg['actv_size']), dtype='float32')

        h5_pointer = 0
        for batch in tqdm(loader, total=int(cfg['store_size'] / cfg['batch_size'])):
            if h5_pointer + cfg['batch_size'] > cfg['store_size']:
                dset[h5_pointer:] = batch[:cfg['store_size'] - h5_pointer].cpu().numpy()
                break
            else:
