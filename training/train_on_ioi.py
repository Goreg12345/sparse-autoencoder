...
text_dataset_loader = iter(
    DataLoader(
        text_dataset, batch_size=None, shuffle=False, num_workers=1, prefetch_factor=200
    )
)
...
reconstruction_loss_metric = ReconstructionLoss(
    llm,
    sae,
    TextDataset(
        dataset, llm.to_tokens, 16, drop_last_batch=False, seq_len=cfg["seq_len"]
    ),
    cfg["actv_name"],
)
...
wandb_logger = pl.loggers.WandbLogger(project="serimats", config=cfg, name=f"sae_debug")
